%**********  Function decription ***********
% This is a leave-one-out cross-validation (LOO-CV) experiments, which is used to calculate the accuracy and precision of the calibration results of ICP algorithm. 
...
%*******************************************
function [Min_AE, Mean_AE, Max_AE, SD, sRMSE] = ICP_cross_validation_experiments(eul_P_R2ER, eul_P_R2EL, eul_P_I2Ntip, rotm_ER2Ntip, rotm_EL2Ptip, plane_type)
    ...
    for sample_points = exp_nums : 1: exp_nums % excute the exp_i-fold-cross-validation by order. sample_points must greater than 4.
        ...
        for test_i = 1 : sample_points
            ...
            exp_eul_P_R2EL = eul_P_R2EL(1:sample_points,:); % Selection the experimental data
            ...
            [rotm_I2PTip,reg_rsme]= ICP_calibration(exp_eul_P_R2ER, exp_eul_P_R2EL, exp_eul_P_I2Ntip,rotm_ER2Ntip, rotm_EL2Ptip, plane_type); % Get the results of calibration
    ...
    %     startIndex_of_results = startIndex_of_results + 4;
        disp("***ICP algorithm: Exp.data = "+ sample_points + ": Min_AE, Mean_AE, Max_AE, SD, sRMSE ***");
        disp([Min_AE, Mean_AE, Max_AE, SD, sRMSE]);
        sheetfile  = ['Sheet',num2str(sample_points)];
        xlswrite("Exp_results\ICP_Accuracy_20200807", roundn(results,-4),sheetfile); % write the result to xls
    end    
end